...
function haveChild(){
  ...
  const firstLine = lateBirth ? bucketPick(latePool,'child_late') : bucketPick(base,'child');
...
function observe(){
  ...
  // Ambient observations fire in nearly every life, so they were the loudest cross-session staleness signal.
  // Two fixes: (1) obsPick gives them their OWN deep anti-repeat memory (see core.js) so a line can't resurface
  // for many lives; (2) a per-life cap (P._obsN) stops the back half of a long life from FLOODING with ambient
  // texture (jurors "stopped reading" it). The pools below are also deepened on the highest-frequency triggers.
  P._obsN=P._obsN||0;
  const ob=(key,cond,pool,cls)=>{ if(!firedObs[key]&&cond&&P._obsN<4){firedObs[key]=1;P._obsN++;logLine(Array.isArray(pool)?obsPick(pool):pool,cls||'obs');} };
  if(a>40) ob('vit_tire',s.vit<35,["The stairs have started to ask a question of {them}.","The body has begun keeping its own quiet counsel.","{Their} legs know something the rest of {them} is not yet ready to hear.","Mornings take longer to arrive in the body than they once did.","{They} has started to plan the day around what the body will permit.","The body has begun sending its small bills, and expecting them paid.","Some mornings the body answers the bell late, and grumbling.","{They} has started taking the stairs the way one takes a difficult conversation.","The weather has begun living in {their} joints, and forecasting badly.","{They} no longer trusts the body to do, unasked, what it always used to.","There is a small daily ceremony now to standing up, and {they} performs it carefully.","The body has begun rounding {their} ambitions down to what it can actually deliver.","{They} has noticed the handrail becoming, quietly, a thing {they} uses.","Distances {they} once never thought about have started introducing themselves.","The morning stiffness has stopped being news and become the weather {they} wakes into.","{They} has begun to hear {their} own joints offering opinions on the day.","Things that used to take no thought have started asking for a little, and then a little more.","The body keeps reminding {them}, gently and then less gently, that it is not twenty."]);
  if(a>55) ob('vit_neg',s.vit<18,["{Their} body has become a small daily negotiation.","Each day now asks a little more of {them} than the last one did.","The body keeps its own ledger now, and the sums are getting harder.","Getting up has become the first real work of every day.","The body has stopped being something {they} has, and become something {they} tends.","{They} has grown courteous with {their} own body, the way one is with the powerful.","The body sets the day's terms now, and {they} has learned to accept them.","{They} negotiates with the body each morning, and loses a little more of each round.","What the body will allow has become the shape the day is poured into.","The body has become a country {they} travels with increasing care.","Every plan {they} makes now passes, first, through the body's narrow gate.","{They} has learned the exact inventory of what the day's strength will buy.","The body asks for rest the way a creditor asks for payment, and {they} pays.","{They} measures distances now in whether the body will forgive them.","The simplest movements have acquired a cost {they} pays without comment.","{They} has made a slow peace with a body that no longer keeps its old promises."]);
  ob('mind_hi',s.mind>78&&a>20,["Books have become a country {they} can live in.","The mind has become a room {they} can close the door of.","Reading has stopped being something {they} does, and has become somewhere {they} goes.","{They} has grown used to being the most interested person in the room.","Thinking has become {their} favourite weather.","There is a whole life {they} keeps behind the eyes, and visits often.","{They} has furnished, behind the eyes, a country no one else can quite get into.","An idea, in {them}, is a place to go and not come back from for hours.","{They} has learned to be alone without being lonely, which the mind makes possible.","Curiosity has worn {them} a set of grooves the years keep deepening.","{They} has accumulated, quietly, an interior the size of a small country.","A good question, to {them}, is better company than most people.","{They} has built behind the eyes a place the weather never reaches.","The inside of {their} own head has become {their} favourite place to spend an afternoon.","{They} keeps a running argument with the world going, and enjoys it enormously.","Thinking has stayed, for {them}, the most reliable pleasure there is."]);
  ...
  ob('spirit_hi',s.spirit>88&&a>30,["{They} still laughs like someone the years have not yet learned how to reach.","The decades have arranged themselves around {them} and somehow never sat down.","Somewhere in {them} a window the years usually close has stayed open.","Whatever the years took, they did not take the lightness in {them}.","{They} has kept, against most of the evidence, a stubborn appetite for the days.","Time has not yet managed to talk {them} out of being glad.","{They} wears the years lightly, the way some people wear a coat they like.","There is a brightness in {them} the hard seasons kept failing to put out.","{They} has stayed, somehow, on speaking terms with joy.","The world has handed {them} the usual griefs, and {they} has gone on being glad anyway.","{They} has held onto a delight in small things the years kept trying to confiscate.","Gladness, in {them}, has turned out to be a more durable material than anyone expected.","{They} greets the ordinary day like someone who has decided, on principle, to enjoy it.","The light still gets into {them} the way it got into the child {they} were.","{They} has the rare trick of being old and largely unbruised by it.","Something in {them} keeps refusing, cheerfully, to be discouraged.","{They} laughs at the right things still, and at the wrong ones, which is better."]);
...
function eligible(){
  ...
  return CARDS.filter(c=>{
    ...
    // once-per-DYNASTY. Numeric onceDyn values are explicit generational cooldowns
    // for the few cards that should recur later in a long house.
    if(c.onceDyn && typeof S!=='undefined' && S && S.seenDyn && S.seenDyn[c.id]!=null){
      if(c.onceDyn===true) return false;
      if((P.gen - S.seenDyn[c.id]) < c.onceDyn) return false;
    }
...
function drawCard(){
  ...
  if(!P.flags.vocation){
    // most lives are nudged firmly into a calling (the engine of vocation variety) — but a minority are
    // decided, once and silently, to be the life that never picks a single shape. Those drift uncalled and
    // meet their own distinct cluster (y_adrift -> the adult/elder drifter arc), so "no calling" is a real
    // fifth life-shape with its own content, not an empty hole where a vocation should be.
    if(P.flags.noCalling==null) P.flags.noCalling = chance(0.13);
    if(!P.flags.noCalling){
      const vc=pool.find(c=>c.id==='y_calling');
      if(vc && ((vc.age && P.age>=vc.age[1]-4) || chance(0.5))){ presentCard(vc); return; }
    }else{
      // the drifter gets the SAME near-certainty the called life does — so a noCalling life reliably reaches
      // y_adrift (which sets vocation='adrift' and unlocks the drifter arc) instead of silently arc-less.
      const ad=pool.find(c=>c.id==='y_adrift');
      if(ad && ((ad.age && P.age>=ad.age[1]-3) || chance(0.45))){ presentCard(ad); return; }
    }
  }
  ...
  const SIG=/^(a_soldier|m_soldier|e_soldier|a_scholar|m_scholar|e_scholar|a_maker|m_maker|e_maker|a_wanderer|m_wanderer|e_wanderer|a_adrift|m_adrift|e_adrift|vx_|t_|w_)/;
...
function presentCard(c){
  ...
  if(c.onceDyn && typeof S!=='undefined' && S){ S.seenDyn=S.seenDyn||{}; S.seenDyn[c.id]=P.gen; }
